...
def load_data(*args, **kwargs):
    ...
    # Pass the output_path to the test function
    return output_path


@test
def test_output(output_path, *args) -> None:
    """
    Test the output of the block.
    This test will check if the file was created successfully and if it's not empty.
    """
    import os
    import pandas as pd
    ...
    # Carica il file CSV e controlla se contiene dei dati
    df = pd.read_csv(output_path)
    assert not df.empty, "The generated CSV file is empty."
    
    print(f"Test passed. File {output_path} exists and contains data.")
